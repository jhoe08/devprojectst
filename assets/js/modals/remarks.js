...
if(createRemarks) {
  ...
  createRemarks.addEventListener('click', async () => {
    try {
        let selectedStatus = document.querySelector('#selectedStatus');
        ...
        console.log({createRemarks,selectedStatus, selectedPeriod, transid})
      ...
      console.log(error)
        notifyCustom('exclamation', `Field is empty please check!asdadas`, `${error}`, 'danger')
    }
  })
}
